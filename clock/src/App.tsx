...
const App = () => {
  ...
  const [countrySelected, setCountrySelected] = useState<CountryData>({
    name: "",
    timezone_offset: 0,
    latlong: "",
  });
  ...
  const countryFiltered = (value: string) => {
    const inputValue = value.trim().toLowerCase();
    const inputLength = inputValue.length;

    const filterCountry = countryData.filter((country: CountryData) => {
      const completeText = country.name + " " + country.name;
    ...
    return inputLength === 0 ? [] : filterCountry;
  };

  const selectedCountry = (country: CountryData) => {
  ...
  const eventEnter = (event: any) => {
    if (event.key === '13') {
      ...
      let entered: { timezone_offset: number; name: string; latlong: string } =
        {
          name: split[0].trim(),
          timezone_offset: split[1].trim(),
          latlong: split[2].trim(),
        };
      setCountrySelected(entered);
    }
  };
  ...
  /* useEffect(() => {
    ...
    if (countrySelected.name !== "") {
      let interval = setInterval(() => {
        setClock(countrySelected.name, countrySelected.timezone_offset);
      }, 1000);
    } else {
  ...
  const setClock = (city: string, offset: number) => {
    const currentDate = new Date();
    const utcDate =
      currentDate.getTime() + currentDate.getTimezoneOffset() * 60000;
    const cityDate = new Date(utcDate + 3600000 * offset);
    let secondRatio = cityDate.getSeconds() / 60;
    let minuteRatio = (secondRatio + cityDate.getMinutes()) / 60;
    let hourRatio = (minuteRatio + cityDate.getHours()) / 12;
    setTime({
      cityDate: cityDate.toLocaleTimeString(),
      secondRatio,
      minuteRatio,
      hourRatio,
    });
  };
  const { secondRatio, minuteRatio, hourRatio } = time;
    ...
    <h1>What time it is ?</h1>
      <Autosuggest
        ...
        onSuggestionsFetchRequested={({ value }) => {
          setValue(value);
          setCountrySelected(countryFiltered(value)[0]);
        }}
        onSuggestionsClearRequested={() => setCountry([])}
        getSuggestionValue={(suggestion: CountryData) =>
          `${suggestion.name} - ${suggestion.timezone_offset}`
        }
        renderSuggestion={(suggestion: CountryData) => (
          <div onClick={() => selectedCountry(suggestion)}>
            {`${suggestion.name} - ${suggestion.timezone_offset}`}
          </div>
        )}
        inputProps={{
